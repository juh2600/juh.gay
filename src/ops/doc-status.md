...
- A document in `draft` status may:
	- (Owner) Be [sent to `review`](#review-procedure).
	- (Owner) Be [marked as `obsolete`](#obsoletion).
- A document in `review` may:
	...
	- (Reviewer) Be [`approved` by the reviewer](#review:execute).
- An `approved` document may:
	- (Author) Receive minor edits and be republished without review.
	- (Author) Receive major edits and be [sent back to `review`](#review:init).
	- (Owner) [Become `deprecated`](#deprecation) and planned for obsoletion.
- A `deprecated` document may:
	- (Owner) Be [sent back to `review`](#review:init) to see if it can be made relevant again.
	- (Owner) [Become `obsolete`](#obsoletion) once ALL documents that depend on it have had those dependencies removed.
...
2. [Create an issue](https://github.com/josephreed2600/juh.gay/issues/new?assignees=&labels=documentation%2C+review-request&template=peer-review-request.md&title=%5BReview%5D+src%2Fpath%2Fto%2Ffile.md) on the site's GitHub using the review template.
